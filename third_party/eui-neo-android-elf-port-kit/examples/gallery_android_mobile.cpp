...
std::string sampleInput = "你好，EUI-NEO 😉";
std::string sampleEditor = "你好，EUI-NEO 😉\n可以在这里输入多行文本。";
...
std::string sampleFeedback = "就绪";
float pageScroll[6] = {};
float bingCarouselPosition = 0.0f;

constexpr float kSidebarWidth = 210.0f;
constexpr float kNavTop = 86.0f;
constexpr float kNavHeight = 42.0f;
constexpr float kNavGap = 10.0f;

double galleryTimeSeconds() {
...
eui::Transition pageTransition() {
    ...
    return eui::Transition::make(0.32f, eui::Ease::OutCubic);
}
...
const char* pageSubtitle() {
    if (selectedPage == 1) {
        return "面向开发者的文字层级、图标文字与主题色令牌。";
    }
    if (selectedPage == 2) {
        return "由 DSL 过渡驱动的点击、悬停与动画示例。";
    }
    if (selectedPage == 3) {
        return "使用同一套矩形和文字图元构建的交互设置。";
    }
    if (selectedPage == 4) {
        return "在同一页面中展示必应每日图片与 API 文本请求。";
    }
    if (selectedPage == 5) {
        return "一个轻量、优雅的 C++ GUI 框架。";
    }
    return "在一个界面中展示基础控件、状态和视觉属性。";
}
    ...
    components::button(ui, id)
        ...
        .onClick([page] {
            if (selectedPage != page) {
                selectedPage = page;
                pageSwitchTime = galleryTimeSeconds();
            }
        })
        .build();
}
        ...
        .content([&] {
            ...
            ui.column("sidebar.content")
                ...
                .content([&] {
                    ...
                    navItem(ui, "nav.controls", "控件", 0xF1B2, 0);
                    navItem(ui, "nav.text", "样式", 0xF1FC, 1);
                    navItem(ui, "nav.animation", "动画", 0xF2F1, 2);
                    navItem(ui, "nav.bing", "必应", 0xF1C5, 4);
                    navItem(ui, "nav.settings", "设置", 0xF013, 3);
                    navItem(ui, "nav.about", "关于", 0xF05A, 5);
                });

            ui.stack("sidebar.theme")
                .x(18.0f)
                .y(std::max(0.0f, height - 64.0f))
                .size(174.0f, 42.0f)
                .content([&] {
...
std::string bingApiText() {
    ...
    if (!result.ready) {
        return "正在加载必应 API 文本...";
    }
    if (!result.ok) {
        return "网络文本请求失败。";
    }
    const std::string copyright = jsonStringValue(result.body, "copyright");
    return copyright.empty() ? "必应 API 已返回文本数据。" : copyright;
}
...
void composeSettingsPage(eui::Ui& ui, float width, float height) {
    ...
    ui.column("settings.list")
        ...
        .gap(14.0f)
        .content([&] {
            settingRow(ui, "setting.dense", "紧凑布局", "为图库页面使用更紧凑的间距。", optionDense, rowWidth, [] { optionDense = !optionDense; });
            settingRow(ui, "setting.glass", "玻璃表面", "在控件页显示透明面板示例。", optionGlass, rowWidth, [] { optionGlass = !optionGlass; });
            settingRow(ui, "setting.motion", "动画过渡", "保持页面和属性过渡动画启用。", optionMotion, rowWidth, [] { optionMotion = !optionMotion; });
            settingRow(ui, "setting.unlockFps", "解除 90 FPS 限制", "允许动画渲染使用显示器刷新率。", optionUnlockFps, rowWidth, [] { optionUnlockFps = !optionUnlockFps; });
            settingRow(ui, "setting.night", "夜间模式", "在浅色与深色主题令牌之间切换图库。", optionNight, rowWidth, [] { optionNight = !optionNight; });
        });
}
...
void composeContent(eui::Ui& ui, float width, float height) {
    const float shellWidth = std::max(0.0f, width - 48.0f);
    const float innerWidth = std::max(0.0f, shellWidth - 44.0f);
    const float shellHeight = std::max(0.0f, height - 48.0f);
    const float innerHeight = std::max(0.0f, shellHeight - 44.0f);
    const float headerGap = optionDense ? 12.0f : 18.0f;
    const float bodyHeight = std::max(0.0f, innerHeight - 38.0f - 24.0f - headerGap * 2.0f);
    const int page = std::clamp(selectedPage, 0, 5);
    const double switchElapsed = optionMotion ? std::max(0.0, galleryTimeSeconds() - pageSwitchTime) : 1.0;
    const float switchProgress = optionMotion ? std::clamp(static_cast<float>(switchElapsed / 0.32), 0.0f, 1.0f) : 1.0f;
    const float switchEase = 1.0f - (1.0f - switchProgress) * (1.0f - switchProgress) * (1.0f - switchProgress);
    const float pageEnterOffset = (1.0f - switchEase) * 18.0f;
    const float pageEnterOpacity = 0.72f + switchEase * 0.28f;
    const float pageEnterVeil = optionGlass ? (1.0f - switchEase) * 0.10f : 0.0f;
    ...
    ui.stack("content.area")
        ...
        .content([&] {
            ...
            ui.rect("page.shell")
                ...
                .radius(42.0f)
                .border(1.0f, borderColor())
                .shadow(34.0f, 0.0f, 18.0f, shadowColor(0.30f, 0.16f))
                .transition(pageTransition())
                .build();
...
void compose(eui::Ui& ui, const eui::Screen& screen) {
    ...
    components::dialog(ui, "feedback.dialog")
        ...
        .title("对话框组件")
        .message("用于聚焦确认的模态界面。它与图库其余部分使用相同的主题令牌、按钮和脏区域渲染路径。")
        .primaryText("确认")
        .secondaryText("取消")
        .onPrimary([] {
