...
#if defined(EUI_WINDOW_BACKEND_ANDROID)
    addUniqueExtension(extensions, VK_KHR_SURFACE_EXTENSION_NAME);
    addUniqueExtension(extensions, VK_KHR_ANDROID_SURFACE_EXTENSION_NAME);
#elif defined(EUI_WINDOW_BACKEND_SDL2)
...
bool VulkanRenderBackend::createSurface() {
#if defined(EUI_WINDOW_BACKEND_ANDROID)
    const core::window::NativeWindowInfo info = core::window::nativeWindowInfo(window_);
    auto* nativeWindow = static_cast<ANativeWindow*>(info.platformWindow);
    if (nativeWindow == nullptr) {
        std::fprintf(stderr, "EUI Vulkan: Android native window is null\\n");
        return false;
    }
    ...
    if (createAndroidSurface == nullptr) {
        std::fprintf(stderr, "EUI Vulkan: vkCreateAndroidSurfaceKHR was not found\\n");
        return false;
    }

    return createAndroidSurface(instance_, &createInfo, nullptr, &surface_) == VK_SUCCESS;
#elif defined(EUI_WINDOW_BACKEND_SDL2)
